...
| D11 | Container image delivery | `docker save` tars in the data partition image at build time; `docker load` on first boot, then delete the tars | PoC spec §6. Sizing consequence in R5 — peak usage is ~2× the image set, so the data partition must be sized for it |
...
| D22 | Common image bases | Prefer **Debian Bookworm** when an existing official multi-arch variant supports the required service: use `php:8.4-apache-bookworm` for web and `redis:7.0-bookworm` for Redis. Keep `mariadb:11.8-noble` and `eclipse-mosquitto:2.0-openssl` as upstream exceptions. | Image inspection verifies Bookworm amd64/arm64 variants for PHP and Redis. Official MariaDB uses Ubuntu and Mosquitto 2.0 OpenSSL is Alpine-only. Rebuilding database or broker images just to standardise a base adds ownership without a PoC benefit. Standardise immutable pins, provenance, runtime policy and multi-architecture support across all four. |
...
1. `app/docker-compose.yml` derived from `emoncms-docker`'s, **every image pinned by
   digest** for both architectures (`app/images.lock`), `MARIADB_AUTO_UPGRADE` **removed**
   (learnings §7.3 layer 3 — do not carry a known hazard into a new design), MariaDB as its
   own container (APP-9). Use the fork's GHCR web image from `php:8.4-apache-bookworm` and
   the official `redis:7.0-bookworm` variant. Use the fresh-install baseline
