...
| **0.7 Confirm the Pi 4 bench** — board, SD cards, USB-serial on the GPIO UART, scriptable switchable power | **R12** | **DONE 2026-09-25.** The board, SD card, USB-serial console and physical Ethernet all work; the host has no route to the Pi, so harness access is correctly serial-only (D18). Three identical CH340 adapters mean the console must use `/dev/serial/by-path/`, not `by-id` (Appendix B). After replacing the USB-A-to-C cable, no undervoltage event occurred during a 4-core/120-s host stress test or a full web/MariaDB/Redis/Mosquitto container start plus PHP CPU work. Tasmota at `http://172.16.1.2/`, built-in relay **Power5**, was commanded OFF for 12 s then ON. The Pi lost power, booted normally after restoration, regained DHCP at `172.16.2.253`, reached `running`, started Docker, and passed the 3/3 real serial harness. Keep Power5 feeding the final PSU if hardware changes. |
...
| WP | Work | Cumulative |
|---|---|---|
| WP0 | Risk retirement — **COMPLETE 2026-09-25.** QEMU probes done (R1, R2, F5 corrected); R5 retired by offline preload tests; R6 retired 2026-09-24; the Pi bench cable, workload and Tasmota controlled-power checks pass (0.7) | complete |
...
| Pi 4 bench (2026-09-25) | Console at 115200 on the CH340 at `/dev/serial/by-path/pci-0000:07:00.0-usb-0:2.3.2.3.4:1.0-port0` (`ttyUSB2` today). The host has three identical CH340s with no serial numbers, so `/dev/serial/by-id/usb-1a86_USB_Serial-if00-port0` points at whichever enumerated last. Do not use it. Physical Ethernet `end0` gets DHCP at `172.16.2.253/22`, gateway/DNS `172.16.1.1`, and registry access; the host cannot route to it, so all test access remains serial (D18). 8 GB RAM. The replacement USB-A-to-C cable completed clean 4-core CPU and container-start/PHP-load trials without a new undervoltage event. Tasmota `http://172.16.1.2/` built-in `Power5` was verified: OFF for 12 s caused Pi power loss; ON caused a normal U-Boot/Linux boot, DHCP recovery and a passing 3/3 serial harness. Keep its outlet in the final PSU path |
