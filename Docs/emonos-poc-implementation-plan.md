...
| **0.7 Confirm the Pi 4 bench** — board, SD cards, USB-serial on the GPIO UART, scriptable switchable power | **R12** | **Nearly complete.** The board, SD card, USB-serial console and Ethernet (DHCP, DNS and registry access) all work. Three identical CH340 adapters mean the console must use `/dev/serial/by-path/`, not `by-id` (Appendix B). The USB-A-to-C cable was replaced after an earlier undervoltage event; on 2026-09-25, a clean 4-core, 120-second CPU saturation test generated **no** new undervoltage event. Repeat under the full stack before closing the PSU item. Switchable power exists: Tasmota at `http://172.16.1.2/`, built-in relay **Power5** (reported ON); it must be moved to the final PSU's power path if the PSU changes, then command/recovery behavior must be tested before T7. |
...
| Pi 4 bench (2026-09-25) | Console at 115200 on the CH340 at `/dev/serial/by-path/pci-0000:07:00.0-usb-0:2.3.2.3.4:1.0-port0` (`ttyUSB2` today). The host has three identical CH340s with no serial numbers, so `/dev/serial/by-id/usb-1a86_USB_Serial-if00-port0` points at whichever enumerated last. Do not use it. Ethernet `end0` gets DHCP on 172.16.0.0/22 and reaches the internet. The host has no route to it, so all access goes through the serial console (D18). 8 GB RAM. An earlier container-load `Undervoltage detected!` event prompted replacement of the USB-A-to-C cable; clean 4-core CPU saturation for 120 s generated no new event. Re-test once the full stack is restored. Tasmota at `http://172.16.1.2/` reports built-in `Power5` ON; use it for scriptable PSU switching only after confirming it controls the final PSU path |
