...
run_x86_64_vm: emonos_x86_64_vm
	qemu-system-x86_64 -machine q35 -accel kvm -cpu host -m 1024M -display none -serial mon:stdio -bios $(X86_OUTPUT)/images/OVMF.fd -blockdev driver=file,filename=$(X86_OUTPUT)/images/emonos-x86-64-vm.img,aio=threads,node-name=vmfile -blockdev driver=raw,file=vmfile,node-name=vmdisk -device virtio-blk-pci,drive=vmdisk -nic user,model=virtio-net-pci
